...
const REMOVED_COLUMN_KEYS = ["extras", "wardrobe", "ref"];
const MARKER_TYPES = ["call", "meal", "move", "note"];

// Old free-text `timing` ("9:00 AM" or "9:00 AM – 9:20 AM") -> {timeAnchor, timeDuration}.
function migrateTiming(s) {
  if (typeof s.timeAnchor !== "string") s.timeAnchor = "";
  if (typeof s.timeDuration !== "number") s.timeDuration = 20;
  if (s.timing && !s.timeAnchor) {
    const parts = String(s.timing).split(/[–-]/).map((p) => p.trim());
    s.timeAnchor = parts[0] || "";
  }
}

function migrate(data) {
  data.scenes.forEach((s) => {
    if (s.kind !== "marker") s.kind = "shot";
    if (s.kind === "marker") {
      if (typeof s.label !== "string") s.label = "";
      if (!MARKER_TYPES.includes(s.markerType)) s.markerType = "note";
    }
    if (!Array.isArray(s.images)) s.images = [];
    if (s.colorTag === undefined) s.colorTag = null;
    if (typeof s.intExt !== "string") s.intExt = "";
    if (typeof s.dayNight !== "string") s.dayNight = "";
    if (typeof s.dayNightIntExt !== "string") s.dayNightIntExt = "";
    if (typeof s.timing !== "string") s.timing = "";
    migrateTiming(s);
    if (!s.customFields || typeof s.customFields !== "object") s.customFields = {};
  });
  data.projects.forEach((p) => {
    if (typeof p.autoNumber !== "boolean") {
      p.autoNumber = false;
    }
    ...
    if (!Array.isArray(p.columnOrder) || !p.columnOrder.length) {
      p.columnOrder = DEFAULT_COLUMN_ORDER;
    } else {
      const validKeys = new Set([...DEFAULT_COLUMN_ORDER, ...p.customColumns.map((c) => c.key)]);
      let order = p.columnOrder.filter((k) => !REMOVED_COLUMN_KEYS.includes(k));
      let mergedDayNight = false;
      order = order.flatMap((k) => {
        if (k === "intExt" || k === "dayNight") {
          if (mergedDayNight) return [];
          mergedDayNight = true;
          return ["dayNightIntExt"];
        }
        return [k];
      });
      if (!order.includes("timing")) {
        order = ["timing", ...order];
      }
      p.columnOrder = order.filter((k) => validKeys.has(k));
    }
  });
  return data;
}
...
// Serializes an entire read-modify-write cycle so concurrent requests (e.g.
// several PATCHes firing within the same debounce window) can't each read
// the file before an earlier one has written, silently dropping updates.
// `fn` receives the freshly-read data and should mutate + return it.
export function withData(fn) {
  ...
  // Keep the chain alive even if one operation throws, so later ones still run.
...
export function mkMarker(projectId, dayId, order) {
  return {
    id: randomUUID(),
    projectId,
    dayId,
